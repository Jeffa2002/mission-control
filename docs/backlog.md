...
4. `[todo]` Make incident controls real and persistent.
...
5. `[todo]` Replace placeholder overview/status contracts.
...
6. `[todo]` Update Activity to use the GitHub deploy feed.
...
7. `[todo]` Improve `/deploys` operator usefulness.
...
8. `[todo]` Reduce panel container blast radius.
...
9. `[todo]` Stop disabling SSH host verification.
...
10. `[todo]` Add a working verification gate before deploy.
...
11. `[todo]` Clean generated/runtime artifacts out of Git.
...
12. `[todo]` Split shared data helpers for deploys, agents, systems, and activity.
