...
import { estimateAnthropicCost, estimateOpenAiCost, parseCodexRollout, parseUsageTranscript, serializeUsageSse } from './usage-model.ts';
...
test('prices Anthropic input, output and cache tokens', () => assert.equal(estimateAnthropicCost('claude-sonnet-5', { input: 1_000_000, output: 1_000_000, cacheRead: 1_000_000, cacheWrite: 1_000_000 }), 16.2));
test('parses and prices Anthropic transcript usage', () => { const rows = parseUsageTranscript('{"type":"message","timestamp":"2026-08-06T00:00:00Z","message":{"role":"assistant","provider":"anthropic","model":"claude-opus-4-6","usage":{"input":10,"output":2,"cacheRead":20,"cacheWrite":5,"totalTokens":37}}}', 'designer'); assert.equal(rows.length, 1); assert.ok(rows[0].estimatedCost > 0); });
